...
function Promoters(props) {
	...
	useEffect(() => {
		...
		const evnts = async () => {
			let tempLst = [];
			let response = await getEventsByPromoter({
	...
	useEffect(() => {
		if (showUpdateMessageBar || showLoginMessageBar || showDeleteMessageBar) {
	...
	return (
		<div style={{ backgroundColor: '#0c3c60' }}>
				...
				<p className={styles.message}>
					{showLoginMessageBar && (
						<div style={{ width: '14.25rem' }}>
							{' '}
							<MessageBar message={'Logged in successfully'} />
						</div>
					)}
				</p>
				<p className={styles.message}>
					{showUpdateMessageBar && (
						<div style={{ width: '17rem' }}>
							{' '}
							<MessageBar message={'Event updated successfully'} />
						</div>
					)}
				</p>
				<p className={styles.message}>
					{showDeleteMessageBar && (
						<div style={{ width: '16.5rem' }}>
							{' '}
							<MessageBar message={'Event deleted successfully'} />
						</div>
					)}
				</p>
															...
															<Grid.Column>
																<Icon
																	onClick={async (e) => {
																		...
																		const eventToBeEditedResponse =
																			await getEvent({ id: Number(event.id) });
                    ...
                    <section >
                      <div {...getRootProps()}>
                        ...
                        <p>Drag 'n' drop some files here, or click to select files</p>
											...
											<StyledDropzone
												eventInfo={eventToUpdateInfo}
												setEventInfo={seteventToUpdateInfo}
											/>
										</Form.Input>
									</Form.Group>
								</Form>
								...
								onClick={async () => {
									...
									if (date || details || location || photo || price || title) {
										if (
											...
											// If I remove all letters from location and after it is an empty string location needs update.
											// If it not empty string location is in lat & lng meaning it has not changed
											if (location.replace(/[A-Za-z]/g, '') === '') {
												const geoCodeResponse = await fetch(
													`https://maps.googleapis.com/maps/api/geocode/json?address=${location}, Puerto Rico&key=${process.env.REACT_APP_MAP_KEY}`
												).then((response) => response.json());
												const { lat, lng } =
